function [xnew, pos,mu,nbiter,obj4] = monqp_ELM(H,c,C,l,verbose)
...
if nargin < 4		% default value for the regularisation parameter 
    l = 0;				 
end; 

if nargin < 5		% default value for the display parameter 
    verbose = 0;				 
end; 
if  size(C,1)~=nl
    %   keyboard
...
%     end;
    ...
    if (sum(xnew< 0) > 0 | sum(xnew > C(ind)) > 0)    %  03/01/2002 AR
